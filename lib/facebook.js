...
exports.getPagePosts = function(data,start,end,done) {
  ...
  var doFetch = function(url) {
    oauth.facebookOAuth().get(url,data.auth.facebook,function(err,body) {
      if (err) {
        done(err);
      } else {
        var data = JSON.parse(body);
  ...
  var firstURL = URLRoot + data.facebookPage + '/feed?' + querystring.stringify({
...
exports.doPageCall = function(data,start,end,process,done) {
  if (data.auth.facebook && data.facebookPage) {
