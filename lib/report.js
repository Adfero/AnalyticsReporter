...
exports.calculateScore = function(url,inData,outData) {
  var total = reporters.reduce(function(previous,current) {
    if (outData.averages[current.name] !== false && outData.averages[current.name] != 0 && outData.pages[current.name][url.path]) {
  ...
  var number = reporters.reduce(function(previous,current) {
    return previous + (outData.averages[current.name] !== false && outData.pages[current.name][url.path] ? 1 : 0);
  },0.0);
