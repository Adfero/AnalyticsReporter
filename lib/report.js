        ...
        return function(inData, outData, callback) {
          async.parallel({
            'average': function(callback) {
              reporter.average(inData,function(err,averages) {
                if (err) {
                  callback(err);
                } else if (averages && averages.length > 0) {
                  var falses = averages.reduce(function(previous,current) {
                    return current === false ? 1 : 0;
                  },0);
                  if (falses == averages.length) {
                    callback(null,false);
                  } else {
                    callback(null,(averages.reduce(function(a,b) { 
                      return a + b; 
                    },0.0) / inData.sampleDateSegments.length));
                  }
                } else {
                  callback(null,false);
                }
...
exports.calculateScore = function(url,inData,outData) {
  var total = reporters.reduce(function(previous,current) {
    if (outData.averages[current.name] !== false && outData.averages[current.name] != 0 && outData.pages[current.name][url.path] >= 0) {
  ...
  var number = reporters.reduce(function(previous,current) {
    return previous + (outData.averages[current.name] !== false && outData.pages[current.name][url.path] >= 0 ? 1 : 0);
  },0.0);
