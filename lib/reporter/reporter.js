...
class Reporter {
  ...
  generateReport(benchmarkStartDate,benchmarkEndDate,reportStartDate,reportEndDate,benchmarkURLs,reportURLs,done) {
    ...
    if (benchmarkLength % reportLength == 0) {
    ...
    async.waterfall([
      ...
      function(benchmarkData,next) {
        var averages = {};
        benchmarkData.forEach(function(outerRow) {
          outerRow.forEach(function(row) {
      ...
      function(averages,next) {
        _this.generateReportData(reportStartDate,reportEndDate,reportURLs,function(err,reportData) {
